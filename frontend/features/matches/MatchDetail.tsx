            ...
            <div className="matchPlayerGrid">
              {playerSlots.map((player, index) => (
                <article className={player.role === "vacant" ? "vacant" : "confirmed"} key={`${player.role}-${index}`}>
                  {player.role === "vacant" ? <span className="vacantNumber">+{index + 1}</span> : <span className="playerPortrait" style={player.avatarUrl ? { backgroundImage: `url(${player.avatarUrl})` } : undefined}>{!player.avatarUrl && <UserCircle aria-hidden="true" weight="fill" />}</span>}
